...
const SUPERADMIN_EMAIL =
  process.env.DEFAULT_SUPERADMIN_EMAIL || "superadmin@example.com";
...
const SUPERADMIN_USERNAME =
  process.env.DEFAULT_SUPERADMIN_USERNAME || "superadmin";
...
async function ensureSuperAdmin() {
  ...
  if (!user) {
    ...
    console.log(
      "ℹ️ Superadmin user already exists, updating baseline fields...",
    );
...
async function ensureImportsTester() {
  ...
  if (!user) {
    ...
    console.log(
      "ℹ️ Imports tester user already exists, updating baseline fields...",
    );
  ...
  const opsRole = await Role.findOne({
    key: "manager_ops",
    scope: "org",
    org_id: DEFAULT_ORG_ID,
  });
