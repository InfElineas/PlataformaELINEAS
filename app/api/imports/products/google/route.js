...
import { importProductsFromValues, persistImportJob } from '@/lib/imports/products';
...
function mergeStats(target, source) {
  ...
  if (target.preview.length < MAX_PREVIEW_ROWS) {
    target.preview.push(...source.preview.slice(0, MAX_PREVIEW_ROWS - target.preview.length));
  }
  return target;
}
...
export async function POST(request) {
  try {
    ...
    if (!fileId) {
      return NextResponse.json({ ok: false, error: 'Debes indicar el ID o URL del documento' }, { status: 400 });
    }
    ...
    return NextResponse.json({ ok: true, stats: aggregated, jobId: job._id.toString() });
  } catch (error) {
    if (error instanceof ImportValidationError) {
      return NextResponse.json({ ok: false, error: error.message, details: error.details }, { status: 400 });
    }
