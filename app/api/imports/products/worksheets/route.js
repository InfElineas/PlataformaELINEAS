...
export async function GET(request) {
  try {
    ...
    const rawId = searchParams.get('fileId') || searchParams.get('spreadsheetId') || '';
    const fileId = extractGoogleFileId(rawId);
    const preferOAuth = parseBoolean(searchParams.get('useOAuth'), false);

    if (!fileId) {
      return NextResponse.json({ ok: false, error: 'Debes indicar un archivo de Google' }, { status: 400 });
    }

    const { token } = await resolveGoogleAccessToken({ userId: session.user.id, preferOAuth });
