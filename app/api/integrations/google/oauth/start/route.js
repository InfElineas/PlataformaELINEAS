...
export async function GET(request) {
  if (!isOAuthConfigured()) {
    return NextResponse.json({ ok: false, error: 'Google OAuth no está configurado en el entorno' }, { status: 400 });
  }

  await requirePermission(request, PERMISSIONS.IMPORTS_MANAGE, { orgScoped: false });
