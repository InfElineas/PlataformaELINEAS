...
import { buildSessionPayload, applySessionCookie, clearSessionCookie, getSessionFromRequest } from '@/lib/auth/session';
...
export async function createSessionResponse({ user, rememberMe = false, request, response }) {
...
export async function requirePermission(request, permission, { orgScoped = true } = {}) {
  ...
  if (permission && !isSuperAdmin && !access.permissionKeys.includes(permission)) {
    throw new ForbiddenError();
  }

  const overrideOrgId = request.headers.get('x-org-id') || request.headers.get('x-org');
...
export async function registerUser({ org_id, email, full_name, username, password, roles = [] }) {
  await connectDB();
...
async function resolveRoles(roleKeys, orgId) {
  const globalRoles = await Role.find({ key: { $in: roleKeys }, scope: 'global' }).lean();
...
export async function logAuditEvent({ org_id, user_id, action, resource, resource_id, meta, request }) {
  await AuditLog.create({
    org_id,
    user_id,
    action,
