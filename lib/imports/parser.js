...
function normalizeRow(parsedRow, rowIndex, options) {
  ...
  const existencia_fisica = sanitizeNumber(parsedRow.existencia_fisica, {
    allowFloat: false,
  });
  const reserva = sanitizeNumber(parsedRow.reserva, { allowFloat: false });
  const disponible_tienda = sanitizeNumber(parsedRow.disponible_tienda, {
    allowFloat: false,
  });
  ...
  const no_almacen = (parsedRow.no_almacen ?? "").toString().trim();
