import { PRODUCT_REQUIRED_FIELDS, MAX_ERROR_DETAILS } from '@/lib/imports/constants';
import { ImportValidationError } from '@/lib/imports/errors';
import { deriveCategoriesFromSheetTitle, ensureTemplateHeaders, resolveFieldFromHeader, sanitizeNumber } from '@/lib/imports/utils';
...
function detectFieldPresence(headers) {
  ...
  const missing = PRODUCT_REQUIRED_FIELDS.filter((field) => !present.has(field));
...
function normalizeRow(parsedRow, rowIndex, options) {
  ...
  if (stock === null) errors.push(`Existencia física inválida en fila ${rowIndex}`);
  ...
  const payload = {
    ...
    derived_categories: options.deriveCategories ? deriveCategoriesFromSheetTitle(options.sheetTitle) : []
  };
...
export function normalizeProductRows(values, { hasHeaders = true, headerOverride, sheetTitle = 'Sheet1', deriveCategories = false } = {}) {
  if (!values?.length) {
  ...
  for (let i = startIndex; i < values.length; i++) {
    ...
    try {
      const result = normalizeRow(parsed, i + 1, { sheetTitle, deriveCategories });
  ...
  if (rowErrors.length) {
    throw new ImportValidationError('Errores de validación en filas', { errors: rowErrors });
  }
