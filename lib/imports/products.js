...
function buildProductDocument(record, { orgId, source, userId }) {
  ...
  const existencia_fisica = toNumberOr(record.existencia_fisica, 0);
  const reserva = toNumberOr(record.reserva, 0);
  const disponible_tienda = toNumberOr(record.disponible_tienda, 0);
  const price = toNumberOr(record.price, 0);

  const no_almacen =
    toStringOrNull(record.no_almacen, record.warehouse_code) || undefined;
  const warehouse_code =
    toStringOrNull(record.warehouse_code, no_almacen) || undefined;
