...
function buildProductDocument(record, { orgId, source, userId }) {
  const product_code = toStringOrNull(record.product_code);
  const name = toStringOrNull(record.name);
  const supplier_name = toStringOrNull(record.supplier_name);
  const store_external_id = toStringOrNull(record.store_external_id);
  const online_category = toStringOrNull(record.online_category);
  const category_aux = toStringOrNull(record.category_aux);
  const units_per_box = toNumberOr(record.units_per_box, 1);

  const existencia_fisica = toNumberOr(
    record.existencia_fisica ?? record.physical_stock,
    0,
  );
  const reserva = toNumberOr(record.reserva ?? record.reserve_qty, 0);
  const disponible_tienda = toNumberOr(
    record.disponible_tienda ?? record.store_qty,
    0,
  );
  const price = toNumberOr(record.price, 0);

  const no_almacen =
    toStringOrNull(record.no_almacen, record.warehouse_code, record.almacen) ||
    undefined;
  const warehouse_code =
    toStringOrNull(record.warehouse_code, no_almacen, record.almacen) ||
    undefined;
  const warehouse_name =
    toStringOrNull(record.warehouse_name, no_almacen, warehouse_code) || "—";
