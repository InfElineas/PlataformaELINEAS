...
import { MAX_PREVIEW_ROWS, PRODUCT_TEMPLATE_HEADERS } from '@/lib/imports/constants';
...
async function upsertProduct(doc, { replaceExisting }) {
  ...
  const existing = await Product.findOne({ org_id: doc.org_id, product_code: doc.product_code });
  if (existing) {
    return 'duplicate';
  }
...
export async function importProductsFromValues(values, { orgId, userId, replaceExisting = true, sheetTitle, source, parserOptions = {} }) {
  try {
    const normalized = normalizeProductRows(values, {
      hasHeaders: parserOptions.hasHeaders !== false,
      headerOverride: parserOptions.headerOverride || (parserOptions.hasHeaders === false ? PRODUCT_TEMPLATE_HEADERS : undefined),
      sheetTitle,
...
export async function persistImportJob({ orgId, userId, source, fileName, sheetName, sheetId, sheetCount, stats, meta = {} }) {
  await connectDB();
