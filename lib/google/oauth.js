...
import { GOOGLE_BASE_TOKEN_URL, GOOGLE_OAUTH_SCOPES } from '@/lib/google/constants';
...
export async function storeUserGoogleTokens(userId, tokens) {
  ...
  user.integrations.google = {
    refresh_token: tokens.refresh_token || user.integrations?.google?.refresh_token,
...
export async function deleteUserGoogleTokens(userId) {
  await connectDB();
  await User.updateOne({ _id: userId }, { $unset: { 'integrations.google': '' } });
}
...
export async function ensureUserAccessToken(userId) {
  ...
  if (integration.access_token && integration.expiry_date && integration.expiry_date > Date.now() + 60000) {
