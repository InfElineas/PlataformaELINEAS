...
import { GOOGLE_BASE_TOKEN_URL, GOOGLE_REQUIRED_SCOPES, GOOGLE_UPLOAD_SCOPES } from '@/lib/google/constants';

let cachedServiceAccountToken = null;
let cachedUploadToken = null;

function readServiceAccountConfig() {
  const inline = process.env.GOOGLE_SERVICE_ACCOUNT_JSON_INLINE || process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
...
async function requestServiceAccountToken(scopes, config) {
  ...
  const assertion = jwt.sign(payload, config.private_key, { algorithm: 'RS256' });
...
export async function getServiceAccountAccessToken({ allowUpload = false } = {}) {
