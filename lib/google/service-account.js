...
function readServiceAccountConfig() {
  ...
  if (inline) {
    try {
      ...
      console.error(
        "[service-account] Error parseando GOOGLE_SERVICE_ACCOUNT_JSON_INLINE:",
        err,
      );
    }
  }
  ...
  const clientEmail =
    process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || inlineObj?.client_email || null;
  ...
  if (
    ...
    console.error(
      "[service-account] private_key no parece una clave RSA válida",
    );
    return null;
  }
