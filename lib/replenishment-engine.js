...
export async function resolveRules(orgId, storeId, categoryId, productId) {
  const rules = await ReplenishmentRule.find({
    ...
    $or: [
      ...
      { 'scope.store_id': storeId, 'scope.category_id': null, 'scope.product_id': null },
      { 'scope.store_id': null, 'scope.category_id': null, 'scope.product_id': null }
    ]
  }).sort({ priority: -1 });
  ...
  const categoryRule = rules.find(r => r.scope.category_id === categoryId && !r.scope.product_id);
  if (categoryRule) return categoryRule.params;

  const storeRule = rules.find(r => r.scope.store_id === storeId && !r.scope.category_id && !r.scope.product_id);
  if (storeRule) return storeRule.params;

  const globalRule = rules.find(r => !r.scope.store_id && !r.scope.category_id && !r.scope.product_id);
 ...
 */
export async function computeAvgDailyDemand(orgId, productId, storeId, windowDays, planDate) {
 ...
 */
export async function calculateReplenishment(orgId, product, storeId, planDate) {
  // Get rules for this product
  const rules = await resolveRules(orgId, storeId, product.category_id, product._id.toString());
  ...
  // Apply seasonality
  const seasonalityFactor = currentSeasonalityMultiplier(planDate, rules.seasonality);

  // Calculate target stock
  const targetBase = (rules.days_of_cover + rules.lead_time_days) * demand * seasonalityFactor + (rules.safety_stock || 0);
  let target = Math.max(rules.min_stock || 0, Math.ceil(targetBase));

  // Get current stock
  const currentStock = await getOnHand(orgId, product._id.toString(), storeId, planDate);
...
export async function generateReplenishmentPlan(orgId, storeId, planDate) {
  ...
  for (const product of products) {
    const calculation = await calculateReplenishment(orgId, product, storeId, planDate);
    planItems.push({
      org_id: orgId,
